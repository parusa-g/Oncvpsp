...
!GIAN: I edit this subroutine to include vo = vh + vxc
...
subroutine run_plot(lmax,np,fp,ep,lloc,rc,npx,lpx,fpx,epx,nxtra, &
&                    vkb,evkb,nproj,rr,vfull,vp,vpuns,vo,zz,mmax,drl,nrl, &
&                    rho,rhoc,rhomod,pswf,srel,cvgplt)
...
!vpuns  unscreened vp
!vo   hartree plus xc potential (GIAN)
!zz  atomic number
...
!Input variables
 ...
 real(dp) :: vo(mmax) ! GIAN added this
...
!Local variables
 ...
 ! ---------------------------------------------------
 do ii=n1,n2
   write(6,'(a, 2(f20.10,1x))') '  !Vae',rr(ii),vfull(ii)
 end do

 do ii=n1,n2
   write(6,'(a, 2(f12.7,1x))') '  !Vhxc',rr(ii),vo(ii)
 end do
 ! ---------------------------------------------------

 ! GIAN edit this part to output vp and vpuns
 if(lloc .eq. 4) then
   do ii=n1,n2
     write(6,'(a, 2(f12.7,1x))') ' !L',rr(ii),vpuns(ii,lloc + 1)
   end do
   do ii=n1,n2
     write(6,'(a, 2(f12.7,1x))') ' !vt',rr(ii),vp(ii,lloc + 1)
   end do
 end if
